...
export const getTodosByUserId = async (
	...
	try {
		...
		if (!user_id || typeof +user_id !== "number")
			return next(ApiError.BadRequest("Некорректный user id"));
...
export const createTodo = async (
	...
	try {
		...
		if (
			...
			return next(
				ApiError.BadRequest(
					"Некорректные данные, user_id: number, title: string не длиннее 30",
				),
			);
...
export const deleteTodo = async (
	...
	try {
		...
		if (!id || typeof +id !== "number")
			return next(
				ApiError.BadRequest(
					"Некорректные данные, id должен быть числом",
				),
			);
...
export const updateTodoTitle = async (
	...
	try {
		...
		if (
			...
			return next(
				ApiError.BadRequest(
					"Некорректные данные, id должен быть числом, title - строкой короче 30",
				),
			);
